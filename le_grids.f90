...
module le_grids
  ...
  ! mapping from g_lo to lz_lo
  type (redist_type), save :: lorentz_map
  ! mapping from g_lo to e_lo
  type (redist_type), save :: energy_map
  ! mapping from g_lo to le_lo
  type (redist_type), save :: le_map
 ...
 ! knobs
  ...
  logical :: lzmapinit = .false.
  logical :: emapinit = .false.
  logical :: lemapinit = .false.
...
!    ! initialize mapping from g_lo to e_lo
!    ! needed for lowflow terms
!    ! and could later replace same initialization
!    ! within collisions.fpp
!    call init_elo_redistribute
    ! initialize mapping from g_lo to le_lo
    ! needed for lowflow terms
    ! and could later replace some initialization
    ! within collisions.fpp
...
!    logical :: done = .false.
    ...
    do ip = 0, nproc-1
       nullify (to_list(ip)%first, from_list(ip)%first)
       nullify (to_list(ip)%second, from_list(ip)%second)
       nullify (to_list(ip)%third, from_list(ip)%third)
       nullify (to_list(ip)%fourth, from_list(ip)%fourth)
    end do
  ...
  subroutine init_elo_redistribute
    use mp, only: nproc
    use species, only: nspec
    use theta_grid, only: ntgrid
    use kt_grids, only: naky, ntheta0
...
!  Problem arose on Pascali compiler (York) with  to_list(ip)%third and fourth
    do ip=0, (nproc-1)
       nullify(to_list(ip)%first,from_list(ip)%first,to_list(ip)%second, &
            from_list(ip)%second,to_list(ip)%third,from_list(ip)%third, &
            to_list(ip)%fourth,from_list(ip)%fourth)
    end do
!<DD
	
! MAB: why is this done twice?
!DD, March 2009: Nullify pointers on initialisation, so do not pass association
!                test when not allocated. 
!  Problem arose on Pascali compiler (York) with  to_list(ip)%third and fourth
    do ip=0, (nproc-1)
...
!<DD
  ...
  end subroutine init_elo_redistribute
  ...
  subroutine init_g2le_redistribute
    ...
    do iglo = g_lo%llim_world, g_lo%ulim_world
       ...
       do ig = -ntgrid, ntgrid
          ...
          ! +2 instead of +1 below because both signs of vpa are going to be on proc
    ...
    call init_redist (le_map, 'c', to_low, to_high, to_list, from_low, from_high, from_list)
