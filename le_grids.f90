  ...
  subroutine init_weights
    ...
    if (vgrid) then
       ...
       do ipt=1,nesub
          ...
          werr(nesub+1:,ipt) = w(nesub+1:negrid-1,1)
...
!          lpe = 2.*lpe/vcut
          ...
          ! get value of first negrid-1 legendre polynomials
          ! at each of the grid points on (0,x0)
          call legendre_polynomials (0.0,x0,zeroes,lpe(:lpesize,:))
       end if
