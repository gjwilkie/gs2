...
! done earlier for energy space
    ...
    do ipt=1,ng2
       ...
       do il=1,ng2-2
          wxerrtmp(il) = lmodzeroes(il) - lmodzeroes(il+1)
          wlerr2tmp(il) = wlerr2tmp(il) + wxerrtmp(il)
          wlerr2tmp(il+1) = wlerr2tmp(il+1) + wxerrtmp(il)
       end do
       ...
       do il=1,ng2-1
          ...
          write(wgt_unit,*) 'pts= ', lmodzeroes(il), 'wgts= ', wlerrtmp(il), ipt, tmpsum
       end do
       write(wgt_unit,*)
  ...
  subroutine get_weights (llim, ulim, nodes, wgts)
    ...
    integer :: npts, rmndr, ndiv, basepts, divrmndr, base_idx, idiv, epts
    ...
    wgts = 0.0; epts = npts; basepts = nmax; divrmndr = 0

    if (nmax .ge. npts) then
       call get_intrvl_weights (llim, ulim, nodes, wgts)
    else
       rmndr = mod(npts-nmax,nmax-1)
    
       if (rmndr == 0) then
          ndiv = (npts-nmax)/(nmax-1) + 1
          allocate (divpts(ndiv))
          divpts = nmax
       else
          ndiv = (npts-nmax)/(nmax-1) + 2
          allocate (divpts(ndiv))
          epts = npts + ndiv - 1
          basepts = epts/ndiv
       ...
       if (first) then
          call get_unused_unit (div_unit)
          call open_output_file (div_unit,".div")
          write (div_unit,*) npts, ndiv, epts, basepts, divrmndr
          do idiv=1,ndiv
             write (div_unit,*) divpts(idiv)
          end do
       end if
       ...
       do idiv=1,ndiv
          ...
          if (first) write (div_unit,*) idiv, base_idx+1, base_idx + divpts(idiv), llim, ulim
  ...
  subroutine get_intrvl_weights (llim, ulim, nodes, wgts)
    ...
    allocate (gnodes((size(nodes)+1)/2), gwgts((size(wgts)+1)/2), omprod((size(nodes)+1)/2))
...
!b MAB
  subroutine integrate_test (g, total, istep)
...
!   over all velocity
    ...
    real, dimension (:), allocatable, save :: xpts2
...
!b MAB
  subroutine lint_error (g, total, istep)
    ...
    real, dimension(:,:,:), allocatable, save :: wlmod, wlmod2
    logical, save :: first = .true.
    real :: fac
          ...
          do ipt = 1,ng2
             do il = 1, ng2
                do ig = -ntgrid, ntgrid
                   ...
                   wlmod2(ig,il,ipt) = wlerr2(il,ipt)*sqrt((bmag(ig)/bmax) &
                        *((1.0/bmax-al(il))/(1.0/bmag(ig)-al(il))))
                end do
             end do
          end do
                      ...
                      if (idx_local (g_lo, iglo)) then
                         do ig = -ntgrid, ntgrid
                            total(ig, it, ik, is, ipt) = total(ig, it, ik, is, ipt) + &
                                 fac*wlmod2(ig,il,ipt)*cos(istep*0.1*xx(il))!*(g(ig,1,iglo)+g(ig,2,iglo))
                         end do
                      end if
                   end do
                end do
