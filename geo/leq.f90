...
contains
  ...
  subroutine leqin(R0, Ra, k, kp, d, dp, r, dr, s, qq, qs, pp, nt_used)
        
    real :: R0, Ra, k, kp, d, dp, r, dr, s, qq, qs, pp
    integer :: nt_used
...
!     to get grad(pressure), multiply grad(psi) by dpressure/dpsi
    ...
    if(char == 'R') then
       call inter_d_cspl(nr, eqpsi, pressure, 1, rp, aa, daa)
       do i=-nth_used, nth_used
          grad(i,1)=grad(i,1)*daa*0.5*beta_0
          grad(i,2)=grad(i,2)*daa*0.5*beta_0
       enddo
    endif
...
! get thet on theta mesh
    ...
    mtheta = (/ ( (j-1)*pi/real(nt-1), j=1,nt) /)
