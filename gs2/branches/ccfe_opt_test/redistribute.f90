...
! <TT
  ...
  subroutine c_redist_22 (r, from_here, to_here)
    ...
    use mp, only: iproc
    use gs2_layouts, only: opt_redist_nbk !NOTE THIS WILL NEED REMOVING BEFORE COMMIT
    type (redist_type), intent (in out) :: r
...
!<DD>
  subroutine c_redist_22_mpi_copy_nonblock(r, from_here, to_here)
    ...
    use mp, only: iproc, nproc, send, receive
    use mp, only:  mp_comm, proc0,barrier,waitall,nbsend,nbrecv
    ...
    integer :: i, ip, nrecv,count,ierr,nsend,j,k
...
!<DD>Moved above send buffer fill
    ! !Post receives
    ...
    !          count=count+1
    !          recvlookup(count)=i
    !          CALL MPI_IRECV(r_buff(1:r%to(i)%nn,count),r%to(i)%nn,mpicmplx,i,i,mp_comm,recvhand(count),ierr)
    !       endif
    !    enddo
    ! endif
          ...
          if (r%from(i)%nn>0.and.i.ne.iproc) then
             ...
             do j=1,r%from(i)%nn
                s_buff(j,count)=from_here(r%from(i)%k(j), &
                                              r%from(i)%l(j))
             enddo
!             CALL MPI_ISEND(s_buff(1:r%from(i)%nn,count),r%from(i)%nn,mpicmplx,i,iproc,mp_comm,sendhand(count),ierr)
    ...
    !       !Actually may have been thinking of SSEND this is blocking in the sense that it doesn't
    !       !exit until matching receive has been posterd. This could cause problems as although we
    !       !post all receives in the code above, if we are sending to a proc lagging behind then we
    !       !end up waiting here until that proc catches up. We are unable to proceed with the other 
    !       !sends until then, hence could we wasting time. On the plus side SSEND avoids buffering.
    !       !Actually ISSEND exists to avoid this problem!
    !       count=count+1
    !       CALL MPI_ISEND(s_buff(1:r%from(i)%nn,count),r%from(i)%nn,mpicmplx,i,iproc,mp_comm,sendhand(count),ierr)
    !    endif
    ! enddo
...
!       CALL MPI_WAITALL(nrecv,recvhand,MPI_STATUSES_IGNORE,ierr) 
...
!       CALL MPI_WAITALL(nsend,sendhand,MPI_STATUSES_IGNORE,ierr)
...
!</DD>
  ...
  subroutine c_redist_22_inv (r, from_here, to_here)
    ...
    use mp, only: iproc
    use gs2_layouts, only: opt_redist_nbk !NOTE THIS WILL NEED TO BE REMOVED BEFORE COMMIT
    type (redist_type), intent (in out) :: r
          ...
          if (r%to(i)%nn>0.and.i.ne.iproc) then
             ...
             do j=1,r%to(i)%nn
                s_buff(j,count)=from_here(r%to(i)%k(j), &
                                              r%to(i)%l(j))
             enddo
!             CALL MPI_ISEND(s_buff(1:r%to(i)%nn,count),r%to(i)%nn,mpicmplx,i,iproc,mp_comm,sendhand(count),ierr)
          endif
       enddo
    endif
    ...
    if (nrecv>0) then
       ...
       do i=0,nproc-1
          if (r%from(i)%nn>0.and.i.ne.iproc) then
             count=count+1
             recvlookup(count)=i
!             CALL MPI_IRECV(r_buff(1:r%from(i)%nn,count),r%from(i)%nn,mpicmplx,i,i,mp_comm,recvhand(count),ierr)
...
!AJ point to be considered.
  ...
  subroutine c_redist_32_mpi_copy(r, from_here, to_here)
    ...
    ! to avoid deadlocks
    do idp = 1, nproc-1
       ipto = mod(iproc + idp, nproc) !<DDCOMM>For this idp where are we thinking of sending stuff?
       ipfrom = mod(iproc + nproc - idp, nproc) !<DDCOMM>For this idp where might we get data from?
       iadp = min(idp, nproc - idp) !<DDCOMM>???
       ! avoid deadlock AND ensure mostly parallel resolution
       if (mod(iproc/iadp,2) == 0) then

!<DDCOMM> What could we do to improve things here?
!We could post all sends at once using non-blocking sends.
!Could be an issue if we end up using all the buffer space.
!Receives semm much slower, is this because we are waiting a long time for the sends to arrive?
!The algorithm splits procs into distinct groups, could this be related to why half procs spend a lot
!longer in the the receive then others.
!RECV can accept any message if wanted, and then look at status to see where it came from (not possible for irecv)
!So by posting all sends and then looping over number of expected messages can just receive whatever we get and then
!put it where we like.
!e.g.
!do ip=1,r%nrecv
!call recv(msg,stat)
!do ii=1,r%to(stat%src)%nn
!outarr(k(ii),l(ii))=msg(ii)
!endloops
          ! send to idpth next processor
          if (r%from(ipto)%nn > 0) then
             !<DDCOM> Copy data into buffer of redist object | Could make our own buffer
             do i = 1, r%from(ipto)%nn
                r%complex_buff(i) = from_here(r%from(ipto)%k(i), &
                                              r%from(ipto)%l(i), &
                                              r%from(ipto)%m(i))
             end do
             !<DDCOM>Send args are data, destination and tag | Tag used to identify messages
             call send (r%complex_buff(1:r%from(ipto)%nn), ipto, idp)
          end if

          ! receive from idpth preceding processor
          if (r%to(ipfrom)%nn > 0) then
             !<DDCOMM>Receive args are data, source and tag.
             call receive (r%complex_buff(1:r%to(ipfrom)%nn), ipfrom, idp)
             !<DDCOMM>Unpack the buffer into the output array
             do i = 1, r%to(ipfrom)%nn
                to_here(r%to(ipfrom)%k(i), &
                        r%to(ipfrom)%l(i)) &
                        = r%complex_buff(i)
...
!<DDHACK>
  subroutine c_redist_32_mpi_copy_nonblock(r, from_here, to_here)
    ...
    use mp, only: iproc, nproc, send, receive
    use mp, only:  mp_comm, proc0,barrier
...
! TT>
  subroutine c_redist_33 (r, from_here, to_here)
    ...
    use mp, only: iproc, nproc, send, receive
    use gs2_layouts, only: opt_redist_nbk
    type (redist_type), intent (in out) :: r
