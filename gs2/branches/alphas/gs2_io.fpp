...
!  integer (kind_nf) :: nttotext_dim, nakxext_dim, time_big_dim
  ...
  integer :: he_delfs2_id, he_hs2_id, he_phis2_id, he_dh2dt_id, he_parstream_id, he_vdrift_id
  ...
  integer :: energy_int_wgt_id, lambda_int_wgt_id, f0_id, f0prim_id, df0dE_id
...
# endif
  ...
  subroutine nc_grids
    ...
    use general_f0, only: f0_values, f0prim, df0dE
...
!    use netcdf_mod
    ...
    real, dimension(nlambda):: lwgt_use
    integer :: ik, it
    integer,dimension(2):: start2, count2

    lwgt_use = lwgts(0,:)/Bovervpar(0,:) 
    ...
    status = nf90_put_var (ncid, df0dE_id, df0dE)
    if (status /= NF90_NOERR) call netcdf_error (status, ncid, df0dE_id)
...
# ifdef NETCDF
    ...
    energyt_dim(1) = negrid_dim
    energyt_dim(2) = time_dim
...
!    status = nf90_def_var (ncid, 'egrid', netcdf_real, (/negrid_dim, nspec_dim/), egrid_id)
    ...
    status = nf90_def_var (ncid, 'df0dE', netcdf_real, nenergy_dim , df0dE_id)
    if (status /= NF90_NOERR) call netcdf_error (status, var='df0dE')
    ...
    status = nf90_def_var (ncid, 'lambda_int_wgt', netcdf_real, nlambda_dim , lambda_int_wgt_id)
    if (status /= NF90_NOERR) call netcdf_error (status, var='lambda_int_wgt')
...
!    <phi>
    ...
    if (write_hrate_e) then
       ...
       status = nf90_def_var (ncid, 'he_vdrift', netcdf_real, flux_e_dim, he_vdrift_id)
       if (status /= NF90_NOERR) call netcdf_error (status, var='he_vdrift')
...
# ifdef NETCDF
    ...
    if (fphi > zero) then
       ...
       if (status /= NF90_NOERR) call netcdf_error (status, ncid, es_flux_e_id)
    end if

    if (fapar > zero) then
       status = nf90_put_var (ncid, apar_flux_e_id, mflux, start=start, count=count)
       if (status /= NF90_NOERR) call netcdf_error (status, ncid, apar_flux_e_id)
    end if

    if (fbpar > zero) then
       status = nf90_put_var (ncid, bpar_flux_e_id, bflux, start=start, count=count)
       if (status /= NF90_NOERR) call netcdf_error (status, ncid, bpar_flux_e_id)
    end if

# endif
  end subroutine nc_flux_e
...
# ifdef NETCDF
    ...
    if (fphi > zero) then
       ...
       if (status /= NF90_NOERR) call netcdf_error (status, ncid, es_flux_emu_id)
    end if

    if (fapar > zero) then
       status = nf90_put_var (ncid, apar_flux_emu_id, mflux, start=start, count=count)
       if (status /= NF90_NOERR) call netcdf_error (status, ncid, apar_flux_emu_id)
    end if

    if (fbpar > zero) then
       status = nf90_put_var (ncid, bpar_flux_emu_id, bflux, start=start, count=count)
       if (status /= NF90_NOERR) call netcdf_error (status, ncid, bpar_flux_emu_id)
    end if
...
# ifdef NETCDF
    ...
    real, dimension (negrid, nspec) :: tmps_e
    complex, dimension (ntheta0, naky) :: tmp
    integer, dimension (4) :: start0, count0, start4, count4
    integer, dimension (3) :: start, count, starth, counth, starte, counte
    ...
    if (write_hrate_e) then
       ...
       status = nf90_put_var (ncid, he_parstream_id, tmps_e, start=starte, count=counte)
       if (status /= NF90_NOERR) call netcdf_error (status, ncid, he_parstream_id)

       call he_repack (he, 6, tmps_e)
       status = nf90_put_var (ncid, he_vdrift_id, tmps_e, start=starte, count=counte)
       if (status /= NF90_NOERR) call netcdf_error (status, ncid, he_vdrift_id)
