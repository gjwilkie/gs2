...
!
  ...
  namelist /gs2_diagnostics_knobs/ print_line, print_old_units, print_flux_line, &
         ...
         save_distfn !Extra flag to save adjusted distfn,Energy grids+Vel. grids
...
!                              real(tperp(ig,it,ik,is)/phi0(it,ik))
    ...
    !Extra section to save adjusted distfn, energy grids and vel grids if save_dsitfn set
    IF (save_distfn) THEN
