...
module file_utils
  ...
  public :: trin
  ! logical :: trin
  ! flag denoting if transport input file present
...
! MAB> -- needed for trinity
  ...
  public :: nt_input_unit
  ! function nt_input_unit (nml)
  ! character(*), intent (in) :: nml
  ! integer :: nt_input_unit
  !    Rewind the Trinity input file to start of namelist NML,
  !    and return its unit number

  public :: nt_input_unit_exist
  ! function nt_input_unit_exist (nml,exist)
  ! character(*), intent (in) :: nml
  ! integer :: input_unit
  !    Rewind the Trinity input file to start of namelist NML,
  !    and return its unit number, setexist=.true.
  ...
  public :: init_input_unit, init_nt_input_unit
  public :: nt_num_input_lines
...
! <MAB
  ...
  public :: mp_open_output_file
  ! subroutine mp_open_output_file (unit, ext, iproc)
  ! integer, intent (out) :: unit
  ! character (*), intent (in) :: ext
  ! integer, intent (in) :: iproc
  !    Open file on current processor with name made from list_name (was trin_name)
  !    (without .trin) and EXT appended, and return its unit number in UNIT
  ...
  ! nttrim, and nt_input_unit_no needed for Trinity -- MAB
  character (500), pointer :: run_name, nt_name
  character (500), target :: arun_name, job_name, nttrim
  character (500) :: list_name
  logical :: trin
  integer, save :: input_unit_no, error_unit_no=6, nt_input_unit_no
  integer, parameter :: stdout_unit=6
! TT>
!  integer, save, public :: num_input_lines
! nt_num_input_lines needed for Trinity -- MAB
  integer, save :: num_input_lines, nt_num_input_lines
! <TT

contains

  subroutine init_file_utils (list, input, error, trin_run, name)
    ! <<wkdoc - find out the [[run_name]], and use the run name to determine whether
    ! this is a [[list]] run ''(i.e. a list of runs has been given)'' or a [[Trinity]] run. 
    ! If not, open the error file and call init_input_unit wkdoc>>
    implicit none
    logical, intent (out) :: list
...
!    call run_type (list)
    if (inp .and. .not. present(trin_run)) then
       ...
       ! set trin=T if input ends in ".trin"
       call run_type (list)
    else
       list = .false.
       trin = .false.
...
!    if (list) then
  ...
  subroutine run_type (list)
    ! <<wkdoc - this determines the type of run, by reading the name of the input file
    !  on the command line into [[arun_name]], and then looking at the extension. If 
    ! the extension is ''.list'', then [[list]] is set to ''.true.''; if the extension
    ! is ''.trin'', then [[trin]] is set to true (EH which presumably means that it is 
    ! a [[Trinity]] run). wkdoc>>
...
!!$# if FCOMPILER == _XL_
!    if arun_name ends in .trin, set trin = T
    trin = (l > 5 .and. arun_name(l-4:l) == ".trin")
...
!!$# endif
  ...
  subroutine init_run_name
    ! <<wkdoc this is called for a non [[Trinity]] or [[list]] run -
    ! it checks that the input file name ends in ''.in'', chops 
    ! the extension off and stores it in [[arun_name]]. It 
    ! also assigns the pointer [[run_name]] to [[arun_name]]. wkdoc>>
    implicit none
    integer :: l
...
! <MAB
  ...
  subroutine get_unused_unit (unit)
    ! <<wkdoc - get an unused ''unit number'' for I/O. wkdoc>>
    implicit none
    integer, intent (out) :: unit
! TT>
...
! <TT
  ...
  subroutine open_output_file (unit, ext)
    ! <<wkdoc - open an output file to write (replacing any existing)
    ! whose name is [[run_name]] + [[ext]], and set [[unit]] to the 
    ! ''unit number'' of that output file. wkdoc>>
    implicit none
    integer, intent (out) :: unit
    character (*), intent (in) :: ext
    character (500) :: hack
  ...
  subroutine mp_open_output_file (unit, ext, iproc)
    ...
    character (*), intent (in) :: ext
    integer, intent (in) :: iproc
    character (500) :: hack, proc
    call get_unused_unit (unit)
    write (proc,*) iproc+1
    proc = adjustl(proc)
    hack=list_name(:len_trim(list_name)-5)//trim(proc)//ext
    open (unit=unit, file=trim(hack), status="replace", action="write")
  end subroutine mp_open_output_file
...
! <TT
  ...
  subroutine init_input_unit (open_it)
    ! <<wkdoc open the input file, strip out any comments and 
...
! MAB> -- needed for Trinity
  subroutine init_nt_name (ntname)

    implicit none

    character (len=500), intent (in) :: ntname

    nttrim = trim(ntname)
    nt_name => nttrim

  end subroutine init_nt_name

  subroutine init_nt_input_unit (open_it)
    implicit none
    logical, intent (in) :: open_it
    integer :: in_unit, out_unit, iostat
    character(500) :: line
    ...
    call get_unused_unit (in_unit)
!    open (unit=in_unit, file=trim(nt_name)//".trin", status="old", &
    open (unit=in_unit, file=trim(nt_name), status="old", &
         action="read", iostat=iostat)
    if (iostat /= 0) then
!       print "(a)", "Couldn't open input file: "//trim(nt_name)//".trin"
       print "(a)", "Couldn't open input file: "//trim(nt_name)
    end if

    call get_unused_unit (out_unit)
!    open (unit=out_unit, status="scratch", action="readwrite")
    open (unit=out_unit, file="."//trim(nt_name))

    iostat = 0
    stack_ptr = 0
    nt_num_input_lines = 0
    do
       read (unit=in_unit, fmt="(a)", iostat=iostat) line
       if (iostat /= 0) then
          if (stack_ptr <= 0) exit
       ...
       call strip_comments (line)
       write (unit=out_unit, fmt="(a)") trim(line)
       nt_num_input_lines = nt_num_input_lines + 1
    end do
    close (unit=in_unit)

    nt_input_unit_no = out_unit
  end subroutine init_nt_input_unit

  function nt_input_unit (nml)
    implicit none
    character(*), intent (in) :: nml
    integer :: nt_input_unit, iostat
    character(500) :: line
    intrinsic adjustl, trim
    nt_input_unit = nt_input_unit_no
    if (nt_input_unit_no > 0) then
       rewind (unit=nt_input_unit_no)
       do
          read (unit=nt_input_unit_no, fmt="(a)", iostat=iostat) line
          if (iostat /= 0) then
             rewind (unit=nt_input_unit_no)
             exit
          end if
          if (trim(adjustl(line)) == "&"//nml) then
             backspace (unit=nt_input_unit_no)
             return
          end if
       end do
    end if
    write (unit=*, fmt="('Couldn''t find namelist: ',a)") nml
  end function nt_input_unit

  function nt_input_unit_exist (nml,exist)
    implicit none
    character(*), intent (in) :: nml
    logical, intent(out) :: exist
    integer :: nt_input_unit_exist, iostat
    character(500) :: line
    intrinsic adjustl, trim
    nt_input_unit_exist = nt_input_unit_no
    exist = .true.
    if (nt_input_unit_no > 0) then
       rewind (unit=nt_input_unit_no)
       do
    ...
    exist = .false.
  end function nt_input_unit_exist
