...
!    if (proc0) time_begin = mpi_wtime()
    if (proc0) time_begin = timer()
    call c_redist_32 (map, gin, gout)
!    if (proc0) time_redist = time_redist + mpi_wtime() - time_begin
    if (proc0) time_redist = time_redist + timer() - time_begin
