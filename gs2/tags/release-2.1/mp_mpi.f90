...
contains
  ...
  subroutine broadcast_character (s)
    ...
    include 'mpif.h'
    integer :: ierror
    call mpi_bcast (s, len(s), MPI_CHARACTER, 0, MPI_COMM_WORLD, ierror)
  ...
  subroutine bcastfrom_character (s, src)
    ...
    include 'mpif.h'
    integer :: ierror
    call mpi_bcast (s, len(s), MPI_CHARACTER, src, MPI_COMM_WORLD, ierror)
  ...
  subroutine receive_character (s, src, tag)
    ...
    tagp = 0
    if (present(tag)) tagp = tag
    call mpi_recv (s, len(s), MPI_CHARACTER, src, tagp, MPI_COMM_WORLD, &
         status, ierror)
