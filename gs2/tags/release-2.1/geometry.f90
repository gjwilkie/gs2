module geometry
  ...
  real :: rhoc, rmaj, r_geo, shift, dbetadrho
  ...
  logical :: gen_eq, vmom_eq, efit_eq, ppl_eq, local_eq, in_nt, writelots, equal_arc
  ...
  integer, private :: ntgrid, ntheth, nth, nrgrid, ntheta
...
contains
  ...
  subroutine eikcoefs
    ...
    use eeq, only: efitin, eeq_init => efit_init
...
! Local variables:
    ...
    real :: rtmp, rho, rho1, rho2, rp, rp1, rp2, drhodrp, dqdrho
    real :: dpsidrp, dpsidrho, dqdrp, dsdrptot, drhodpsi
    real :: qval, qval1, qval2, delrp, akpri, shift0, dum
    ...
    character*1 :: char
    integer :: i, j, k, itot, ndum, nthg, n, ierr
...
!     compute the initial constants
    ...
    call check(vmom_eq, gen_eq, efit_eq, ppl_eq, local_eq) 
    ...
    if(iflux /= 1 .and. iflux /= 10) then
       ...
       call leqin(rmaj, R_geo, akappa, akappri, tri, tripri, rhoc, delrho, shift, &
            qinp, s_hat_input, dpressdrho, ntgrid)
...
!          write(99,*) theta(i),Rpos(rgrid(i),theta(i)),Zpos(rgrid(i),theta(i))
    ...
    if(writelots) write(11,*) 'drhodrp=',drhodrp
!
...
!    endif
  ...
  subroutine drift(rgrid, rp, bvector, gradstot,  &
    ...
    real, dimension (-ntgrid:ntgrid, 3) :: bgradtot, pgradtot, igradtot, &
         dummy, dummy1, curve
    real, dimension (-ntgrid:ntgrid, 2) :: pgrad, igrad, bgrad1, bgrad2, bgrad3
    real, dimension (-ntgrid:ntgrid) :: gbdrift1, gbdrift2, cvdrift1, cvdrift2

    
    real, dimension (2*ntgrid + 1) :: dumdum1, dumdum2

    real :: pressure, err, val, val0, bt, dum, r, t, bi
    character*1 char
...
!       write(*,*) 'error in pbarofrho!'
  ...
  subroutine root(f,fval,a,b,xerrbi,xerrsec,nsolv,ier,soln)
    ...
    niter=int(alog(abs(b-a)/xerrbi)/alog(2.))+1
    do i=1,niter
       trial=0.5*(a1+b1)
       f3=f(trial)-fval
       if(f3*f1 > 0.) then
...
! ignore up-down asymmetry for now
    hmaxu = 0.
    rmax = 0.
    do i=0,nth
    ...
    enddo
    a = 0.5*(Rpos(rgrid(0), theta(0))-Rpos(rgrid(nth), theta(nth)))
    e = hmaxu/a
...
!    write(*,*) 'old nth: ',nthsave,' new nth: ',nth
  ...
  subroutine check(veq, geq, eeq, peq, leq)
    logical, intent(in) :: veq, geq, eeq, peq, leq
  ...
  subroutine rtg(rgrid, rp)
    ...
    do i=-nth,nth
!       broot = bound(theta(i))
       rgrid(i)=rfun(rp, theta(i), bound(theta(i)))
!       broot=rgrid(i)
    enddo
