...
!  public :: gidx2lamidx, lamidx2gintidx
  ...
  type :: g_layout_type
     integer :: iproc
     integer, parameter :: naky, ntheta0, nlambda, negrid, nspec
     integer, parameter :: llim_world, ulim_world, llim_proc, ulim_proc, ulim_alloc, blocksize
  end type g_layout_type

  type :: gint_layout_type
     integer :: iproc
     integer, parameter :: naky, ntheta0, negrid, nspec
     integer, parameter :: llim_world, ulim_world, llim_proc, ulim_proc, ulim_alloc, blocksize
  end type gint_layout_type

  type :: geint_layout_type
     integer :: iproc
     integer, parameter :: naky, ntheta0, nspec
     integer, parameter :: llim_world, ulim_world, llim_proc, ulim_proc, ulim_alloc, blocksize
  ...
  type :: lz_layout_type
     integer :: iproc
     integer, parameter :: ntgrid, naky, ntheta0, negrid, nspec
     integer, parameter :: llim_world, ulim_world, llim_proc, ulim_proc, ulim_alloc, blocksize
...
!     integer :: iproc
!     integer :: ntgrid, naky, ntheta0, negrid, nspec
!     integer :: llim_world, ulim_world, llim_proc, ulim_proc, ulim_alloc, blocksize
...
!  type (lambda_layout_type) :: lambda_lo
  ...
  type :: accel_layout_type
     integer :: iproc
     integer, parameter :: ntgrid, nsign, naky, ndky, ny, ntheta0
     integer, parameter :: nx, nxny, nxnky, negrid, nlambda, nspec, nia
     integer, parameter :: llim_world, ulim_world, llim_proc, ulim_proc, ulim_alloc, blocksize
  ...
  type :: accelx_layout_type
     integer :: iproc
     integer, parameter :: ntgrid, nsign, naky, ny, ntheta0, nx, nxny, negrid, nlambda, nspec
     integer, parameter :: llim_world, ulim_world, llim_proc, ulim_proc, ulim_alloc, blocksize
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine init_dist_fn_layouts &
       (ntgridi, nakyi, ntheta0i, nlambdai, negridi, nspeci)
    use mp, only: iproc, nproc
    use fixed
    implicit none
    integer, intent (in) :: ntgridi, nakyi, ntheta0i, nlambdai, negridi, nspeci
    logical, save :: initialized = .false.

    if (initialized) return
    initialized = .true.
    
    if (ntgridi /= ntgrid) then
       write(*,*) 'ntgrid and fixed parameters disagree.'
       stop
    end if

    if (nakyi /= naky) then
       write(*,*) 'naky and fixed parameters disagree.'
       stop
    end if

    if (ntheta0i /= ntheta0) then
       write(*,*) 'ntheta0 and fixed parameters disagree.'
       stop
    end if

    if (nlambdai /= nlambda) then
       write(*,*) 'nlambda and fixed parameters disagree.'
       stop
    end if

    if (negridi /= negrid) then
       write(*,*) 'negrid and fixed parameters disagree.'
       stop
    end if

    if (nspeci /= nspec) then
       write(*,*) 'nspec and fixed parameters disagree.'
       stop
    end if
    ...
    g_lo%blocksize = g_block
    g_lo%llim_proc = g_llim
    g_lo%ulim_proc = g_ulim
    g_lo%ulim_alloc = g_ulim
    ...
    gint_lo%blocksize = gint_block
    gint_lo%llim_proc = gint_llim
    gint_lo%ulim_proc = gint_ulim
    gint_lo%ulim_alloc = gint_ulim
    ...
    geint_lo%blocksize = geint_block
    geint_lo%llim_proc = geint_llim
    geint_lo%ulim_proc = geint_ulim
    geint_lo%ulim_alloc = geint_ulim
  ...
  pure function idx_g (lo, ik, it, il, ie, is)
    ...
    idx_g = ik-1 + lo%naky*(it-1 + lo%ntheta0*(ie-1 + lo%negrid*(il-1 &
         + lo%nlambda*(is-1))))
  end function idx_g
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine init_lorentz_layouts &
    ...
    use mp, only: iproc, nproc
    implicit none
    integer, intent (in) :: ntgridi, nakyi, ntheta0i, nlambdai, negridi, nspeci, ng2
    logical, save :: initialized = .false.
    ...
    lz_lo%blocksize = lz_block
    lz_lo%llim_proc = lz_llim
    lz_lo%ulim_proc = lz_ulim
    lz_lo%ulim_alloc = lz_ulim
...
!    integer, intent (in) :: i
!    is_idx_lambda = 1 + mod((i - lo%llim_world)/lo%naky/lo%ntheta0/lo%negrid, lo%nspec)
...
!    type (lambda_layout_type), intent (in) :: lo
!    integer, intent (in) :: i
!    ie_idx_lambda = 1 + mod((i - lo%llim_world)/lo%naky/lo%ntheta0, lo%negrid)
...
!    type (lambda_layout_type), intent (in) :: lo
!    integer, intent (in) :: i
!    it_idx_lambda = 1 + mod((i - lo%llim_world)/lo%naky, lo%ntheta0)
...
!    type (lambda_layout_type), intent (in) :: lo
!    integer, intent (in) :: i
!    ik_idx_lambda = 1 + mod((i - lo%llim_world), lo%naky)
...
!    type (lambda_layout_type), intent (in) :: lo
!    integer, intent (in) :: ik, it, ie, is
!    idx_lambda = ik-1 + lo%naky*(it-1 + lo%ntheta0*(ie-1 + lo%negrid*(is-1)))
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine init_accel_transform_layouts &
       (ntgridi, nakyi, ntheta0i, nlambdai, negridi, nspeci, nxi, nyi)
    use mp, only: iproc, nproc
    implicit none
    integer, intent (in) :: ntgridi, nakyi, ntheta0i, nlambdai, negridi, nspeci
...
!!! stopped here.  what to do about nproc?  mp or fixed? 
