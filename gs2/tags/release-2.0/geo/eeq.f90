module eeq
  ...
  real, allocatable, dimension (:) :: dummy, efit_R, efit_Z, sefit_R, sefit_Z
  real, allocatable, dimension (:,:) :: efit_psi, sefit_psi
...
!
    ...
    allocate(tmp1(nbbbs), tmp2(nbbbs))
...
!      read(5,2020)(xlim(i),ylim(i),i=1,limitr)
    ...
    deallocate (rbbbs, zbbbs, tmp1, tmp2)
...
! is therefore not completely general.  It is fine for EFIT output.    
  ...
  subroutine gradient(rgrid, theta, grad, char, rp, nth, ntm)
    ...
    real tmp1, tmp2, aa, daa, rp, rp_bar
    integer i, j
...
!     to get grad(pressure), multiply grad(psi) by dpressure/dpsi
    ...
    if(char == 'R') then
       rp_bar = rp
       call inter_d_cspl(nw, psi_bar, pressure, 1, rp_bar, aa, daa)
       grad = grad*daa*0.5* beta_0/psi_N
    endif
...
!     to get grad(pressure), multiply grad(psi) by dpressure/dpsi
    ...
    if(char == 'R') then
       ...
       do i=-nth_used, nth_used
          grad(i,1)=grad(i,1)*daa * 0.5*beta_0/psi_N
          grad(i,2)=grad(i,2)*daa * 0.5*beta_0/psi_N
       enddo
    endif
...
!     write(*,*) efit_Z(jstar),efit_Z(jstar+1)
  ...
  subroutine alloc_module_arrays(np, nw, nh, nws, nhs)
