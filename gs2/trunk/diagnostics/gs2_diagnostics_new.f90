...
contains
  ...
  subroutine create_dimensions
    ...
    call gnostics%dims%kx%add_to_file(gnostics%sfile)
    call gnostics%dims%ky%init("Y", naky, "The ky dimension", "")
    call gnostics%dims%ky%add_to_file(gnostics%sfile)
    !/Parallel space
    call gnostics%dims%theta%init("z", 2*ntgrid+1, "The theta (parallel) dimension", "")
    call gnostics%dims%theta%add_to_file(gnostics%sfile)
    if (box) then 
       ...
       call gnostics%dims%theta_ext%add_to_file(gnostics%sfile)
    end if
    !/Real space
    ...
    call gnostics%dims%time%add_to_file(gnostics%sfile)
    !/Numeric/generic
    call gnostics%dims%ri%init("r", 2, "Real and imaginary components", "")
    call gnostics%dims%ri%add_to_file(gnostics%sfile)
    call gnostics%dims%generic_2%init("2", 2, "Generic 2", "")
    call gnostics%dims%generic_2%add_to_file(gnostics%sfile)
    call gnostics%dims%generic_3%init("3", 3, "Generic 3", "")
    call gnostics%dims%generic_3%add_to_file(gnostics%sfile)
    call gnostics%dims%generic_4%init("4", 4, "Generic 4", "")
    call gnostics%dims%generic_4%add_to_file(gnostics%sfile)
    call gnostics%dims%generic_5%init("5", 5, "Generic 5", "")
    call gnostics%dims%generic_5%add_to_file(gnostics%sfile)
  ...
  subroutine write_dimensions
    ...
    implicit none
    call create_and_write_variable(gnostics, gnostics%rtype, "kx", trim(dim_string(gnostics%dims%kx)),  &
         "Values of kx, the wavenumber perpendicular to the flux surface ", "1/rho_r", akx)
    call create_and_write_variable(gnostics, gnostics%rtype, "ky", trim(dim_string(gnostics%dims%ky)),  &
         "Values of ky, the wavenumber in the direction of grad alpha ", "1/rho_r", aky)
    call create_and_write_variable(gnostics, gnostics%rtype, "theta", trim(dim_string(gnostics%dims%theta)),  &
         "Values of theta, the poloidal angle. ", "rad", theta)
    call create_and_write_variable(gnostics, gnostics%rtype, "energy", trim(dim_string(gnostics%dims%energy)),  &
         "Values of the energy grid. ", "T_s", energy)
    call create_and_write_variable(gnostics, gnostics%rtype, "lambda", trim(dim_string(gnostics%dims%lambda)),  &
