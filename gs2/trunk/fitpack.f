...
c readable documentation.
      ...
      subroutine fitp_curv1 (n,x,y,slp1,slpn,islpsw,yp,temp,
     *                  sigma,ierr)
c
...
c
    9 ierr = 2
      ...
      subroutine fitp_curvs (n,x,y,d,isw,s,eps,ys,ysp,sigma,temp,
     *                  ierr)
c
      integer n,isw,ierr
...
c
      ...
      fitp_curv2 = sum-del1*del2*(yp(i)*(del1+dels)+yp(im1)*
     *        (del2+dels))/(6.*dels)
      return
    1 sigdel = sigmap*dels
      ...
      fitp_curv2 = sum+(yp(i)*del1*(s1-ss)+yp(im1)*del2*(s2-ss))/
     *            (sigdel*sigmap*(1.+ss))
      return
      end
...
c
    ...
    1 sigdel = sigmap*dels
      ...
      fitp_curvd = sum+(yp(i)*(c1-ss)-yp(im1)*(c2-ss))/
     *        (sigdel*sigmap*(1.+ss))
      return
      end
...
c
    8 ierr = 3
      ...
      subroutine fitp_curvps (n,x,y,p,d,isw,s,eps,ys,ysp,sigma,
     *                   temp,ierr)
c
      integer n,isw,ierr
...
c
    ...
    2 sum = (y(i)*del1+y(im1)*del2)/dels
      ...
      fitp_curvp2 = sum-del1*del2*(yp(i)*(del1+dels)+yp(im1)*
     *        (del2+dels))/(6.*dels)
      return
    3 sigdel = sigmap*dels
      ...
      call fitp_snhcsh (s1,dummy,sigmap*del1,-1)
      call fitp_snhcsh (s2,dummy,sigmap*del2,-1)
      fitp_curvp2 = sum+(yp(i)*del1*(s1-ss)+yp(im1)*del2*(s2-ss))/
     *            (sigdel*sigmap*(1.+ss))
      return
      end
...
c                  8603 altus cove, austin, texas 78759, usa
c
c this function is the derivative of curvp2
c interpolates a curve at a given point using
c a periodic spline under tension. the subroutine curvp1
c should be called earlier to determine certain necessary
c parameters.
c
...
c on output--
c
c   curvpd contains the interpolated derivative
c
c none of the input parameters are altered.
c
c this function references package modules intrvp and
...
c
    ...
    3 sigdel = sigmap*dels
      ...
      call fitp_snhcsh (dummy,c1,sigmap*del1,-1)
      call fitp_snhcsh (dummy,c2,sigmap*del2,-1)
      fitp_curvpd = sum+(yp(i)*(c1-ss)-yp(im1)*(c2-ss))/
     *        (sigdel*sigmap*(1.+ss))
      return
      end
...
c
   ...
   21 fitp_curvpi = float(ideltp)*(so+si)+float(isign)*so
      ...
      subroutine fitp_kurv1 (n,x,y,slp1,slpn,islpsw,xp,yp,temp,s,
     *                  sigma,ierr)
c
      integer n,islpsw,ierr
...
c
    ...
    1 sigdel = sigmap*dels
      ...
      subroutine fitp_kurvd (t,xs,ys,xst,yst,xstt,ystt,n,x,y,xp,
     *                  yp,s,sigma)
c
      integer n
...
c
    ...
    1 sigdel = sigmap*dels
      ...
      subroutine fitp_kurvpd (t,xs,ys,xst,yst,xstt,ystt,n,x,y,xp,
     *                  yp,s,sigma)
c
      integer n
      real t,xs,ys,xst,yst,xstt,ystt,x(n),y(n),xp(n),yp(n),
...
c-----------------------------------------------------------
      ...
      integer im1
      real su, sl hsd11, alphap, betap, betapp
c
      if (n .lt. 2) go to 25
      if (s .lt. 0.) go to 26
...
c       subroutine fitp_snhcsh (sinhm,coshm,x,isw)
c c
c       integer isw
...
c c with relative error less than 4.0e-14.
c c
c c on input--
c c
...
c c
c c sinhm approximation on (0.,2.2)
c c
c       sinhm = xs*((((sp14*xs+sp13)*xs+sp12)*xs+sp11)*xs+1.)/
c      .             ((sq12*xs+sq11)*xs+sq10)
c       return
c c
c c sinhm approximation on (2.2,3.9)
c c
...
c c
c c sinhm approximation on (3.9,5.1)
c c
c       xs = ax*ax
...
c c
c c sinhm approximation on (5.1,6.1)
c c
c       xs = ax*ax
...
c       return
c c
c c sinhm approximation above 6.1
c c
c     4 expx = exp(ax)
...
c
    ...
    1 sigdel = sigma*del
      ...
      real a(n),r(n)
c
c Not quite right for non-uniform r mesh
c
      if(iside.eq.1) then
         dedge=-(3.*a(1)-4.*a(2)+a(3))/(r(3)-r(1))
      else
         dedge=(3.*a(iside)-4.*a(iside-1)+a(iside-2))
     .        /(r(iside)-r(iside-2))
      endif
