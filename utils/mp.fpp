...
!<LA shared memory public procedures and data types
  public :: onnode, node_id, get_node_pointer, shm_node_barrier

!LA>

!<LA data types for shared memory
  type c3_aptr_t
     complex, pointer :: p(:,:,:) => null()
  end type c3_aptr_t

  type shm_info_t
     integer comm, size, id ! comm, number of mpi ranks and id in the node
     integer, allocatable :: wranks(:) ! the world ranks of the node ranks
     integer, allocatable :: g_lo_se(:,:) ! start end of g_lo per rank
     type(c3_aptr_t), allocatable :: g_lo_ptr (:)
  end type shm_info_t
...
!<MAB
    ...
    call scope (allprocs)
    call FIPC_node_init(comm_all, ierror)
    if (ierror /= FIPC_success) then
       write(0,*) "FIPC node initalisation failed"
    endif
    call  FIPC_init( comm_all, ierror)
    if (ierror /= FIPC_success) then
       write(0,*) "FIPC context initalisation failed"
    endif
    call init_shm_info
...
# endif
    ...
    contains
      ...
      subroutine init_shm_info
        implicit none

        integer comm, wcomm, id_world, id_node, n, ierr
        call FIPC_ctxt_intra_comm( FIPC_ctxt_world, comm, ierr )
        call  mpi_comm_size(comm, n, ierr)
        call mpi_comm_rank(comm, id_node, ierr)
        shm_info%id = id_node
        shm_info%size = n
        shm_info%comm = comm
        allocate(shm_info%wranks(0:n-1), shm_info%g_lo_se(2,0:n-1))

        call  FIPC_ctxt_world_comm( FIPC_ctxt_world, wcomm, ierr )
        call mpi_comm_rank(wcomm, id_world, ierr)

        call mpi_allgather(id_world, 1, mpi_integer, &
             shm_info%wranks, 1, mpi_integer, comm, ierr)

      end subroutine init_shm_info
...
# ifdef MPI
    ...
    call FIPC_node_finalize(ierror)
...
# endif
  ...
  subroutine shm_node_barrier
# ifdef MPI
    use FIPC_module
    implicit none

    integer ierr
    Call FIPC_node_barrier(FIPC_ctxt_world, ierr)
#endif
...
! cheks if a node rank belong to the curent node
    ...
    if ( ip >= 0 .and. ip < nproc) then
       ...
       call mp_abort("rank value out of rannge in onnode function")
    end if
  ...
  function get_node_pointer(pin)
    implicit none
    complex, target, intent(in) :: pin(:,:,:)
    complex, pointer :: get_node_pointer(:,:,:)
    complex, pointer :: aux(:,:,:) => null()

    integer i, j, s, e

     get_node_pointer => null()
...
! ATTENTION this works only pin is allocated, i..e. all e >= s
 ...
 enddo pointer_array
 if (.not. associated(get_node_pointer)) then
    call mp_abort("get_node_pointer fail to find a node pointer")
 endif
end function get_node_pointer 
